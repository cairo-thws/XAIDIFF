...
parser.add_argument(
    "--classifierguidancescale", type=float, default=0.2, required=False, help="Strength of the classifier guidance"
)
parser.add_argument(
    "--structureguidancescale", type=float, default=1.0, required=False, help="Strength of the structure guidance"
)
parser.add_argument(
    "--affclassifierguidancescale",
    type=float,
    default=0.2,
    required=False,
    help="Strength of the classifier guidance for the affirmatives",
)
parser.add_argument("--adamstepsize", type=float, default=1.0, required=False, help="Strength of the conditioning")
parser.add_argument("--lpdistance", type=int, default=1, required=False, help="lpdistance")
...
SAMPLE_IMAGE_INDICES = [7, 12, 89, 110]  # [0, 1, 2, 4]
...
# Experiment Settings
target_classes = 0  # 0: Class "no soccerball in image", 1: Class "soccerball in image"
classifier_guidance_scale = args.classifierguidancescale  # 0.2
structural_guidance_scale = args.structureguidancescale
structural_guidance_lpdistance = args.lpdistance
adam_step_size = args.adamstepsize

aff_classifier_guidance_scale = args.affclassifierguidancescale
...
assert PRECISION in [
    torch.float32,
    torch.float64,
], "Precision must be torch.float32 or torch.float64"
...
classifier = load_classifier(
    ...
    weights_path=f"pretrained_models/sportballs/model.safetensors",
)
# /REPLACE
...
storage.write_json(
    ...
    {
        "classifier_guidance_scale": args.classifierguidancescale,
        "adam_step_size": args.adamstepsize,
    },
)
